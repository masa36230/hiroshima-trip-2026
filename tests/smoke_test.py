...
def assert_page(page, viewport_name):
    ...
    assert page.locator('#day4 .timeline li:has-text("広島城") time').inner_text() == "12:15"
    ...
    assert page.get_by_text("お好み村").count() >= 3
    assert page.locator('#day4 .timeline li:has-text("お好み村で広島お好み焼き") time').inner_text() == "13:10"
    assert "13:10" in page.locator('[data-booking="okonomiyaki"] + .custom-check + .check-rank + .check-copy small').inner_text()
